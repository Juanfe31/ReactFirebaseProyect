...
import { getEcas, deleteEca, updateEca } from '../api/Eca';
import Swal from 'sweetalert2';
import _ from "lodash";
import MyContext from './context';
import { useContext } from 'react';

export default function ConsultarECA() {
  ...
  const numberArt = 10;
  const pageCount = Math.ceil(ecas.length / numberArt);
  ...
  useEffect(() => {
    const getEcasF = async () => {
      const getEcasApi = await getEcas();
      setEcas(getEcasApi);
    }
    getEcasF();
  }, [])

  function eliminarEca(idEca, nombreEca) {
    Swal.fire({
      title: '¿Estás seguro?',
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire(
          'Borrado!',
          'La ECA "' + nombreEca + '" se ha borrado del sistema',
          'success'
        ).then((r) => {
          if (r.isConfirmed) {
  ...
  function visualizarEca(idEca, eca, documento, nombre_representante, nombre_eca, municipio, direccion) {
    Swal.fire({
      ...
      html:
        ...
        '<input id="swal-inputDocumento" class="swal2-input" readonly value="' + documento + '"> </div>' +
        '<div><label class="swal2-label col-md-2">Representante </label>' +
        '<input id="swal-inputNombre_representante" class="swal2-input" readonly value="' + nombre_representante + '"> </div>' +
        '<div><label class="swal2-label col-md-2">Nombre Eca/ Bodega </label>' +
        '<input id="swal-inputNombre_eca" class="swal2-input" readonly value="' + nombre_eca + '"> </div>' +
        '<div><label class="swal2-label col-md-2">Municipio </label>' +
        '<input id="swal-inputMunicipio" class="swal2-input" readonly value="' + municipio + '"> </div>' +
        '<div><label class="swal2-label col-md-2">Direccion </label>' +
        '<input id="swal-inputDireccion" class="swal2-input" readonly value="' + direccion + '"  > </div>'
      ,
      focusConfirm: false,
  ...
  function editarEca(idEca, eca, documento, nombre_representante, nombre_eca, municipio, direccion) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        updateEca(idEca, eca, result.value[0], result.value[1], result.value[2], newMunicipio, result.value[4])
        Swal.fire(
          'Actualizado!',
  ...
  const filterEcas = ecas?.slice(0, ecas.length).filter((eca) => {
    if (selectValue === 'cc') {
            ...
            <div className='campo-documento'>
              <div className='row justify-content-center tipo-id'>
                ...
                <select className='col-xl-3 col-md-6' name='tipo-documento' id='tipo' onChange={(e) => { setquery(""); setSelectValue(e.target.value); (e.target.value === "mun" ? setShow(true) : setShow(false)) }} value={selectValue}>
                  ...
                  <option value='nom'>Nombre Eca</option>
                </select>
              </div>
            ...
            <table className='col-12 table table-hover' >
              ...
              <tbody>
                {filterEcas?.slice((paginated - 1) * numberArt, (paginated) * numberArt).map((datos, index) => (
                  ...
                  <td>{datos.macrorutas.map(element => 
                    element.label+" "
                  )}</td>
                    <td>{datos.municipio}</td>
                    <td>{datos.direccion}</td>
                    <td >
                      <div style={{ display: 'inline' }} className="deleteBtn mx-1" onClick={() => { eliminarEca(datos.id, datos.nombre_eca) }}><i className="fa-solid fa-trash-can fa-xl"></i></div>
                      ...
                      <div style={{ display: 'inline' }} className="deleteBtn " onClick={() => { visualizarEca(datos.id, datos, datos.documento, datos.nombre_representante, datos.nombre_eca, datos.municipio, datos.direccion) }}><i className="fa-regular fa-eye fa-xl"></i></div>
                    </td>
                  </tr>
                ))}
              </tbody>
